...
def check_password(guess, trials=100):
    ...
    # Log the password being tested
    logging.info("Testing password: %s", guess)
   
    # Run the command several times to capture timing
    for _ in range(trials):
        start_time = time.perf_counter()

        try:
            result = subprocess.run(['./vault.o', guess], capture_output=True, check=True)
    ...
    avg_time = sum(times) / trials if trials > 0 else 0  # Regular average
    variance = statistics.variance(times) if len(times) > 1 else 0.0  # Calculate variance
...
def brute_force_password_by_position(max_length=12, trials=100):
    ...
    for position in range(max_length):
        ...
        # Try each character for the current position
        for char in possible_chars:
            guess = password + char + 'a' * (max_length - len(password) - 1)  # Fill remaining with 'a'
            elapsed_time, variance = check_password(guess, trials)
           
            # Log the timing result
            logging.info("Position %d, Guess %s, Elapsed Time: %.6f, Variance: %.6f",
                         position + 1, char, elapsed_time, variance)
           
            # Track the character with the longest time for each position
            if elapsed_time > best_time:
                best_time = elapsed_time
                best_char = char
                logging.info("Best character at position %d: %s, time: %.6f",
...
# Call the function to start the timing attack per character position
